...
public class GetCountriesHandler : IRequestHandler<GetCountriesQuery, ResponseModel<CursorPagedResponseModel<Country>>>
{
    ...
    {
        var items = await _repository.GetCountriesAsync(request.Cursor, request.PageSize);
        var itemList = items.ToList();
        
        var nextCursor = itemList.LastOrDefault()?.Id;
        var hasMore = nextCursor.HasValue && await _repository.HasMoreCountriesAsync(nextCursor.Value);
