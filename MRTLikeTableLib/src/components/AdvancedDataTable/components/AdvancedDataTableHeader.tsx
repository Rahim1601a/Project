...
function AdvancedDataTableHeaderInner<T extends object>({
  ...
  const totalWidth =
    leftPinnedCols.reduce((acc, c) => acc + c.getSize(), 0) + totalUnpinnedWidth + rightPinnedCols.reduce((acc, c) => acc + c.getSize(), 0);
  ...
  return (
    <ADTHeaderBar role='rowgroup'>
      {table.getHeaderGroups().map((headerGroup) => (
        <ADTHeaderRow key={headerGroup.id} role='row' style={{ width: totalWidth, minWidth: '100%' }}>
          {/* Left Pinned Headers */}
          {headerGroup.headers
          ...
          {virtualColumns.map((virtualColumn) => {
            const column = unpinnedCols[virtualColumn.index];

            if (!column) return null;

            const header = headerGroup.headers.find((currentHeader) => currentHeader.column.id === column.id);
...
export const AdvancedDataTableHeader = AdvancedDataTableHeaderInner;
