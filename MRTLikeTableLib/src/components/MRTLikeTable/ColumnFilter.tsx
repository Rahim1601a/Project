...
import { setFilterSafe } from './filters';
...
const CheckboxFilter = ({ localValue, setLocalValue }: FilterProps) => (
  <Checkbox
    size='small'
    checked={localValue === 'Y'}
    onChange={(e) => setLocalValue(e.target.checked ? 'Y' : undefined)}
    sx={{ mt: 0.5 }}
  />
);
...
export const ColumnFilter = React.memo(function ColumnFilter({
  ...
  const columnFilterOptions = React.useMemo(() => 
    accessorKey ? (filterOptions?.[accessorKey] ?? []) : [],
  [accessorKey, filterOptions]);
