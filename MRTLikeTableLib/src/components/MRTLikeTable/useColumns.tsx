...
import { filterFnByVariant } from './filters';
...
export function useColumns<T extends object>({
  ...
  return useMemo<MRTLikeColumnDef<T>[]>(() => {
    ...
    cols.push(
      ...columns.map((col) => {
        ...
        return {
          ...col,
          filterFn:
            col.filterVariant && filterFnByVariant[col.filterVariant]
              ? filterFnByVariant[col.filterVariant]
              : filterFnByVariant.text,
        };
      }),
    );
          ...
          return (
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', gap: 0.5 }}>
              ...
              {actionMode === 'inline' ? (
                ...
                <RowActionMenu row={row.original} render={renderRowActionMenuItems} />
              )}
            </Box>
          );
        },
