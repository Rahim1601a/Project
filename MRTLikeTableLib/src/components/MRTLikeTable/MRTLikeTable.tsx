...
import type { MRTLikeTableProps, MRTValidationErrors } from './types';
import { filterFnByVariant, buildFilterOptionsFromData } from './filters';
import { MRTLikeTableRow } from './MRTLikeTableRow';
import { MRTLikeTableHeaderCell } from './MRTLikeTableHeaderCell';
import { FIXED_COLUMN_WIDTHS, COLUMN_DEFAULTS, ROW_HEIGHTS, VIRTUALIZER_OVERSCAN, SCROLL_BUFFER, PAGE_SIZE_OPTIONS } from './constants';
import { useColumns } from './useColumns';

/** Lazy-load heavy export dependencies (jsPDF is ~300KB) */
const lazyExportCSV = async (...args: Parameters<typeof import('./export').exportCSV>) => {
  const { exportCSV } = await import('./export');
  return exportCSV(...args);
};
const lazyExportXLSX = async (...args: Parameters<typeof import('./export').exportXLSX>) => {
  const { exportXLSX } = await import('./export');
  return exportXLSX(...args);
};
const lazyExportPDF = async (...args: Parameters<typeof import('./export').exportPDF>) => {
                ...
                : rowVirtualizer.getVirtualItems().map((virtualRow) => {
                    ...
                    return (
                      <MRTLikeTableRow
                        ...
                        onEditChange={(col: string, val: any) => setEditValues((prev) => ({ ...prev, [col]: val }))}
                        columnSizing={table.getState().columnSizing}
...
const MRTLikeTable = memo(MRTLikeTableInner) as typeof MRTLikeTableInner;
